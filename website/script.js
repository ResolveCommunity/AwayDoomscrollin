...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const i18n = {
        en: {
            ...
            vs_row4_away: "Privacy First! Screen analysis stays on-device. Telemetry is disabled by default and runs only after user opt-in; GitHub rule updates run automatically. 100% free and fully open-source.",
            ...
            faq_a1: "Privacy First. Screen-content analysis and blocking run locally. Pseudonymous telemetry is disabled by default and nothing is sent unless the user explicitly opts in; while enabled it sends a random per-installation ID, the disclosed device/app fields, and aggregate blocking statistics to awaydoomscrollin.com. Turning telemetry off stops future submissions and the latest server snapshot expires within 90 days. The ID is not derived from a hardware, advertising, or account identifier. GitHub rule updates run automatically and independently of the telemetry switch. Accessibility permission is used to detect target screens and scrolling gestures.",
            ...
            legal_p2_text: "Screen-content analysis and blocking run on-device. Pseudonymous telemetry is disabled by default and nothing is sent unless the user explicitly opts in. While enabled, it sends a random per-installation ID, device manufacturer/model, Android and SDK version, app version, aggregate platform/total block counts, streak days, and XP to awaydoomscrollin.com. Turning telemetry off stops future submissions; the latest server snapshot expires within 90 days. The installation ID is generated locally, is not derived from a hardware, advertising, or account identifier, and is removed when the App's data is cleared or the App is uninstalled. Separately, the App automatically contacts GitHub at app or accessibility-service startup for rule/configuration updates; successful fetches are cached for six hours and are not controlled by the telemetry switch. Screen contents, messages, passwords, and location are not transmitted.",
        ...
        tr: {
            ...
            vs_row4_away: "Ekran analizi cihazda kalır. Telemetri varsayılan kapalıdır ve yalnızca kullanıcı etkinleştirirse çalışır; GitHub kural güncellemeleri otomatik çalışır. %100 ücretsiz ve tamamen açık kaynak kodludur.",
            ...
            faq_a1: "Gizlilik odaklıdır. Ekran içeriği analizi ve engelleme cihazda çalışır. Takma adlı telemetri varsayılan kapalıdır ve kullanıcı açıkça etkinleştirmedikçe hiçbir telemetri gönderilmez; açıkken rastgele bir kurulum kimliğini, açıklanan cihaz/uygulama alanlarını ve toplu engelleme istatistiklerini awaydoomscrollin.com'a gönderir. Telemetriyi kapatmak gelecekteki gönderimleri durdurur ve son sunucu kaydı en geç 90 gün içinde silinir. Bu kimlik donanım, reklam veya hesap kimliğinden türetilmez. GitHub kural güncellemeleri telemetri anahtarından bağımsız ve otomatik çalışır. Erişilebilirlik izni hedef ekranları ve kaydırma hareketlerini algılamak için kullanılır.",
            ...
            legal_p2_text: "Ekran içeriği analizi ve engelleme cihaz üzerinde çalışır. Takma adlı telemetri varsayılan kapalıdır ve kullanıcı açıkça etkinleştirmedikçe hiçbir telemetri gönderilmez. Açıkken rastgele bir kurulum kimliği, cihaz üreticisi/modeli, Android ve SDK sürümü, uygulama sürümü, platform/toplam engelleme sayıları, seri günleri ve XP awaydoomscrollin.com'a gönderilir. Telemetriyi kapatmak gelecekteki gönderimleri durdurur; son sunucu kaydı en geç 90 gün içinde silinir. Kurulum kimliği yerel olarak üretilir, donanım, reklam veya hesap kimliğinden türetilmez ve uygulamanın verileri temizlendiğinde ya da uygulama kaldırıldığında silinir. Uygulama ayrıca uygulama veya erişilebilirlik servisi başladığında kural/yapılandırma güncellemeleri için GitHub'a otomatik bağlanır; başarılı indirmeler altı saat önbelleğe alınır ve telemetri anahtarından bağımsızdır. Ekran içeriği, mesajlar, şifreler ve konum aktarılmaz.",
