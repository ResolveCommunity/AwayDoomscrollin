...
window.checkDeviceCompatibility = function(brand) {
    ...
    const brandData = {
        samsung: {
            ...
            statusText: isEn ? '<i class="fa-solid fa-flask"></i> Tested Beta Configuration' : '<i class="fa-solid fa-flask"></i> Test Edilmiş Beta Yapılandırması',
            os: 'One UI 4.0 - 6.1',
            title: isEn ? 'Samsung Galaxy Setup Status:' : 'Samsung Galaxy Cihazlar İçin Durum:',
            desc: isEn ? 'Tested on selected Samsung One UI configurations. App, OS, and manufacturer updates can still change accessibility behavior.' : 'Seçili Samsung One UI yapılandırmalarında test edilmiştir. Uygulama, işletim sistemi ve üretici güncellemeleri erişilebilirlik davranışını yine de değiştirebilir.',
            steps: isEn ? [
        ...
        pixel: {
            ...
            desc: isEn ? 'Stock Android is generally less restrictive, but compatibility is still beta and not guaranteed.' : 'Saf Android genellikle daha az kısıtlayıcıdır; ancak uyumluluk yine betadır ve garanti edilmez.',
            steps: isEn ? [
        ...
        other: {
            ...
            desc: isEn ? 'Compatibility varies across Android devices. If the shield stops unexpectedly, review battery optimization and report the device/app versions.' : 'Uyumluluk Android cihazlar arasında değişir. Kalkan beklenmedik şekilde durursa pil optimizasyonunu kontrol edin ve cihaz/uygulama sürümlerini bildirin.',
            steps: isEn ? [
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const i18n = {
        en: {
            ...
            vs_badge_row1_away: "Reflex Breaker (Beta)",
            ...
            vs_row1_away: "The beta shield attempts to recognize an unconscious short-video scroll and return you to the home screen.",
            ...
            vs_row3_away: "Safe-zone rules are designed to exclude direct messages and comments, but third-party UI changes can cause false positives.",
            ...
            vs_row4_away: "Screen analysis stays on-device. Telemetry requires opt-in. GitHub rule requests run automatically even when telemetry is off and currently have no separate opt-out.",
            ...
            hero_subtitle: "Public beta shield for Instagram, TikTok, and YouTube. Detection can vary after platform, Android, or device updates.",
            hero_btn_download: "Download App (APK)",
            hero_btn_calc: "Calculate Your Time",
            hero_spec_samsung: "Public Beta — Compatibility Varies",
            hero_spec_privacy: "Privacy First & Opt-In Telemetry",
            ...
            feat_1_desc: "The beta service attempts to recognize supported short-video feeds and redirect you to the home screen.",
            feat_2_title: "Smart Safe Zone",
            feat_2_desc: "Rules are designed to exclude DMs and comments, but third-party interface changes can cause false positives.",
            ...
            step_3_desc: "When the beta shield recognizes a targeted feed, it attempts to pull you out of the loop.",
            ...
            app_insta_status: "BETA",
            app_insta_desc: "Attempts to detect Reels while excluding messaging and comments; compatibility can change with Instagram updates.",
            app_tiktok_status: "BETA",
            app_tiktok_desc: "Attempts to detect the short-video feed while excluding messaging; compatibility can change with TikTok updates.",
            app_yt_status: "BETA",
            app_yt_desc: "Attempts to detect Shorts while excluding normal videos and Search; compatibility can change with YouTube updates.",
            ...
            comp_samsung_sub: "Tested Configuration — No Universal Guarantee",
            comp_samsung_desc: "Tested on Samsung Galaxy/One UI configurations. App and OS updates can still change accessibility behavior.",
            ...
            faq_a1: "Screen-content analysis runs locally. Pseudonymous telemetry is disabled by default and nothing is sent to awaydoomscrollin.com unless you explicitly opt in. A separate network path automatically requests GitHub rule/configuration data even while telemetry is off; turning telemetry off does not disable it, and there is currently no separate switch. Accessibility permission is used to detect target screens and scrolling gestures.",
            faq_q2: "Will scrolling in Instagram DMs or Comments be blocked?",
            faq_a2: "The beta rules are designed to exclude direct messages and comment sheets, but Instagram interface changes can cause missed detections or false positives. Please report compatibility problems.",
            ...
            legal_p1_text: "At AwayDoomscrollin', we believe digital well-being tools should disclose their behavior clearly. This Privacy Policy explains how the App handles accessibility data, local storage, opt-in pseudonymous telemetry, and automatic GitHub rule updates. We do not sell personal information or use third-party advertising analytics.",
            legal_p2_title: "2. Network Data Collection",
            legal_p2_text: "Screen-content analysis runs on-device. In v1.0.4, pseudonymous telemetry is disabled by default and nothing is sent to awaydoomscrollin.com unless the user explicitly opts in. Opt-in may trigger one immediate snapshot containing the constant PSEUDONYMOUS_TELEMETRY type marker, a random installation ID, disclosed device/App fields, and aggregate blocking, streak, and XP statistics. After that, automatic startup and blocking-event triggers share one persisted 24-hour attempt interval, including failed attempts. Turning telemetry off stops future submissions, although an in-flight request may finish; the latest server snapshot expires within 90 days. Separately, the App automatically requests GitHub rule/configuration data at app or accessibility-service startup and caches successful results for six hours. Turning telemetry off does not disable GitHub requests, and there is currently no separate opt-out. Screen contents, messages, passwords, and location are not transmitted.",
            ...
            legal_p4_text: "Blocking statistics, streak counters, target-app toggles, and recent shield activity are stored locally via SharedPreferences ('away_doomscroll_prefs'). A random telemetry installation ID is created and stored there only after explicit opt-in. These local values are removed when the App's data is cleared or the App is uninstalled. While telemetry is enabled and the attempt interval permits, the installation ID and disclosed aggregate block, streak, and XP fields are also submitted to awaydoomscrollin.com.",
        ...
        tr: {
            ...
            vs_badge_row1_away: "Refleks Kırma (Beta)",
            vs_row1_trad: "\"Günde 15 dk izin ver.\" Beyniniz bu süreyi son saniyesine kadar sömürür ve süre bitince irade savaşı başlar.",
            vs_row1_away: "Beta kalkan bilinçsiz kısa video kaydırmasını algılamaya ve sizi ana ekrana döndürmeye çalışır.",
            ...
            vs_row3_away: "Güvenli alan kuralları DM ve yorumları kapsam dışında tutmak üzere tasarlanmıştır; üçüncü taraf arayüz değişiklikleri hatalı algılamalara yol açabilir.",
            ...
            vs_row4_away: "Ekran analizi cihazda kalır. Telemetri açık rızaya bağlıdır. GitHub kural istekleri telemetri kapalıyken de otomatik çalışır ve ayrı bir kapatma seçeneği yoktur.",
            ...
            hero_subtitle: "Instagram, TikTok ve YouTube için açık beta kalkanı. Algılama platform, Android veya cihaz güncellemelerinden sonra değişebilir.",
            hero_btn_download: "Uygulamayı İndir",
            hero_btn_calc: "Zamanını Hesapla",
            hero_spec_samsung: "Açık Beta — Uyumluluk Değişebilir",
            hero_spec_privacy: "Gizlilik Odaklı & İsteğe Bağlı Telemetri",
            ...
            feat_1_desc: "Beta servis desteklenen kısa video akışlarını algılamaya ve sizi ana ekrana yönlendirmeye çalışır.",
            feat_2_title: "Akıllı Güvenli Bölge",
            feat_2_desc: "Kurallar DM ve yorumları kapsam dışında tutmak üzere tasarlanmıştır; üçüncü taraf arayüz değişiklikleri hatalı algılamalara yol açabilir.",
            ...
            step_3_desc: "Beta kalkan hedef akışı algıladığında sizi sonsuz döngüden çıkarmaya çalışır.",
            ...
            app_insta_status: "BETA",
            app_insta_desc: "Reels akışını algılamaya, mesajlaşma ve yorumları kapsam dışında tutmaya çalışır; Instagram güncellemeleri uyumluluğu değiştirebilir.",
            app_tiktok_status: "BETA",
            app_tiktok_desc: "Kısa video akışını algılamaya, mesajlaşmayı kapsam dışında tutmaya çalışır; TikTok güncellemeleri uyumluluğu değiştirebilir.",
            app_yt_status: "BETA",
            app_yt_desc: "Shorts'u algılamaya, normal videoları ve Arama'yı kapsam dışında tutmaya çalışır; YouTube güncellemeleri uyumluluğu değiştirebilir.",
            ...
            comp_samsung_title: "Samsung Galaxy (One UI)",
            comp_samsung_sub: "Test Edilmiş Yapılandırma — Genel Garanti Yok",
            comp_samsung_desc: "Samsung Galaxy/One UI yapılandırmalarında test edilmiştir. Uygulama ve işletim sistemi güncellemeleri erişilebilirlik davranışını yine de değiştirebilir.",
            ...
            faq_a1: "Ekran içeriği analizi cihazda çalışır. Takma adlı telemetri varsayılan kapalıdır ve açıkça etkinleştirmedikçe awaydoomscrollin.com'a veri gönderilmez. Ayrı bir ağ yolu GitHub kural/yapılandırma verisini telemetri kapalıyken de otomatik ister; telemetriyi kapatmak bu isteği kapatmaz ve şu anda ayrı bir anahtar yoktur. Erişilebilirlik izni hedef ekranları ve kaydırma hareketlerini algılamak için kullanılır.",
            faq_q2: "Instagram DM veya Yorum sayfalarında kaydırmak engellenir mi?",
            faq_a2: "Beta kurallar mesajlaşma ve yorum pencerelerini kapsam dışında tutmak üzere tasarlanmıştır; Instagram arayüz değişiklikleri kaçırılan veya hatalı algılamalara yol açabilir. Uyumluluk sorunlarını lütfen bildirin.",
            ...
            legal_p1_text: "AwayDoomscrollin' olarak dijital refah araçlarının davranışlarını açıkça anlatması gerektiğine inanıyoruz. Bu Gizlilik Politikası erişilebilirlik verilerini, yerel depolamayı, açık rızaya dayalı takma adlı telemetriyi ve otomatik GitHub kural güncellemelerini açıklar. Kişisel bilgileri satmayız ve üçüncü taraf reklam analitiği kullanmayız.",
            legal_p2_title: "2. Ağ Üzerinden Veri İşleme",
            legal_p2_text: "Ekran içeriği analizi cihazda çalışır. v1.0.4'te takma adlı telemetri varsayılan kapalıdır ve kullanıcı açıkça etkinleştirmedikçe awaydoomscrollin.com'a veri gönderilmez. Açık onay; sabit PSEUDONYMOUS_TELEMETRY tür işareti, rastgele kurulum kimliği, açıklanan cihaz/uygulama alanları ile toplu engelleme, seri ve XP istatistiklerini içeren ilk snapshot'ı hemen deneyebilir. Bundan sonra otomatik açılış ve engelleme olayı tetikleyicileri başarısız denemeler dahil kalıcı tek bir 24 saatlik deneme aralığını paylaşır. Telemetriyi kapatmak gelecekteki gönderimleri durdurur, ancak devam eden bir istek tamamlanabilir; son sunucu kaydı en geç 90 gün içinde silinir. Bundan ayrı olarak uygulama veya erişilebilirlik servisi başladığında GitHub kural/yapılandırma verisi otomatik istenir ve başarılı sonuçlar altı saat önbelleğe alınır. Telemetriyi kapatmak GitHub isteklerini kapatmaz ve şu anda ayrı bir kapatma seçeneği yoktur. Ekran içeriği, mesajlar, şifreler ve konum aktarılmaz.",
            ...
            legal_p4_text: "Engelleme istatistikleri, seri sayaçları, hedef uygulama anahtarları ve son kalkan etkinliği SharedPreferences ('away_doomscroll_prefs') ile yerel olarak saklanır. Rastgele telemetri kurulum kimliği yalnız açık onaydan sonra oluşturulup burada saklanır. Bu yerel değerler uygulamanın verileri temizlendiğinde ya da Uygulama kaldırıldığında silinir. Telemetri açıkken ve deneme aralığı izin verdiğinde kurulum kimliği ile açıklanan toplu engelleme, seri ve XP alanları ayrıca awaydoomscrollin.com'a gönderilir.",
